...
createCustomButton("Section 1", "GrinDemon End", "Move to the Trigger Cutscene", function()
    ...
    local rootPart = character and character:FindFirstChild("HumanoidRootPart")
    if rootPart then
        rootPart.CFrame = CFrame.new(-325.581, 15.389, -1113.815)
    end
end)
...
createCustomButton("Section 2", "Dodomeki SafeZone", "Teleport to SafeZone", function()
    local character = localPlayer.Character
    local rootPart = character and character:FindFirstChild("HumanoidRootPart")
    if rootPart then
        rootPart.CFrame = CFrame.new(-325.581, 15.389, -1113.815)
...
-- ====================================================================
-- DOOR PUZZLE SOLVER - CREATE CUSTOM BUTTON FORMAT
-- ====================================================================

createCustomButton("Section 2", "Fix Shape", "Automatically Fix and Submit Shape", function()
    local success, err = pcall(function()
        local detectedCombo = {}
        
        -- Hanapin ang Doors folder
        local doorsFolder = workspace:FindFirstChild("Section2") 
            and workspace.Section2:FindFirstChild("PuzzleDoor") 
            and workspace.Section2.PuzzleDoor:FindFirstChild("Doors")
            
        if not doorsFolder then
            pcall(function() game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Error", Text = "Walang makitang Doors folder!", Duration = 3}) end)
            return
        end
        
        -- I-scan ang mga Combos at shapes sa loob
        for _, desc in ipairs(doorsFolder:GetDescendants()) do
            if desc.Name == "Combos" then
                for _, item in ipairs(desc:GetDescendants()) do
                    if item:IsA("GuiObject") or item:IsA("BasePart") then
                        if item.Name ~= "SurfaceGui" and item.Name ~= "Combos" and not item:IsA("UIListLayout") and not item:IsA("UIGridLayout") then
                            local isAlreadyInside = false
                            for _, existing in ipairs(detectedCombo) do
                                if existing == item.Name then
                                    isAlreadyInside = true
                                    break
                                end
                            end
                            if not isAlreadyInside then
        ...
        -- I-send ang nakitang combos sa server
        if #detectedCombo > 0 then
            ...
            if packetFunc then
                ...
                pcall(function() game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Success!", Text = "Sent: " .. table.concat(detectedCombo, ", "), Duration = 4}) end)
            else
                pcall(function() game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Error", Text = "Walang makitang PacketFunction!", Duration = 3}) end)
            end
        else
            pcall(function() game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Error", Text = "Walang mahanap na shapes sa Combos!", Duration = 3}) end)
        end
    end)
    
    if not success then
        pcall(function() game:GetService("StarterGui"):SetCore("SendNotification", {Title = "System Error", Text = tostring(err), Duration = 4}) end)
    end
end)
