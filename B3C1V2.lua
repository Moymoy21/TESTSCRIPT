...
-- ====================================================================
createCustomButton("Section 1", "Auto Kill Gata (Optional)", "Teleport to Gata and Kill", function()
    local Players = game:GetService("Players")
    local Workspace = game:GetService("Workspace")
    local StarterGui = game:GetService("StarterGui")
...
createCustomButton("Section 1", "Civilian Interact (Optional)", "Teleport to Civilian and Interact", function()
    local player = Players.LocalPlayer
    local character = player.Character or player.CharacterAdded:Wait()
    local humanoidRootPart = character:WaitForChild("HumanoidRootPart")
...
-- ====================================================================
createCustomButton("Section 1", "Teleport to Mika (Optional)", "Go to Mika and Watch Her", function()
...
createToggle("Section 2", "Akari & Akari(Rage) Esp", "Show Esp Name Location of Monster", function(state)
    ...
    if state then
        ...
        notify("IOHUB ACTIVE", "📦 ESP and SafeZone Radar: ON", 3)
    else
        -- ===== [TOGGLE OFF] =====
...
createToggle("Section 3", "Mizuno Box & Name ESP", "Highlights Mizuno and enables the 25-stud escape radar", function(state)
    ...
    if state then
        ...
        notify("IOHUB ACTIVE", "📦 ESP and Radar: ON", 3)
    else
        -- ===== [TOGGLE OFF] =====
...
createCustomButton("Section 4", "Release Terminal", "I-click para i-release ang ShapeTerminal sa Lab CleanseRoom", function()
    task.spawn(function()
        local reliableRemote = game:GetService("ReplicatedStorage")
            :FindFirstChild("modules")
            and game.ReplicatedStorage.modules:FindFirstChild("Packet")
            and game.ReplicatedStorage.modules.Packet:FindFirstChild("Reliable")
            
        if not reliableRemote or not reliableRemote:IsA("RemoteEvent") then
            notify("Error", "Hindi mahanap ang Reliable RemoteEvent!")
            return
        end
        
        local terminal = workspace
            :FindFirstChild("Section4")
            and workspace.Section4:FindFirstChild("Lab")
            and workspace.Section4.Lab:FindFirstChild("CleanseRoomObjective")
            and workspace.Section4.Lab.CleanseRoomObjective:FindFirstChild("ShapeTerminals")
            and workspace.Section4.Lab.CleanseRoomObjective.ShapeTerminals:FindFirstChild("Terminal")
            
        if not terminal then
            notify("Error", "Hindi mahanap ang Terminal sa Section 4!")
            return
        end
        
        -- I-fire ang server para i-release ang terminal
        reliableRemote:FireServer("ShapeTerminal/Release", terminal)
        
        notify("Success", "Matagumpay na na-release ang Terminal! ✅")
    end)
end)
