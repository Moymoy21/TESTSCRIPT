    ...
    if state then
        ...
        task.spawn(function()
            while _G.AutoSafeZoneActive do
                    ...
                    for _, desc in ipairs(Workspace:GetDescendants()) do
                        if desc:IsA("Model") and (desc.Name:lower():find("hogo") or desc.Name:lower():find("guntai")) then
                            ...
                            if monsterRoot then
                                ...
                                -- 25-Stud Threat Radius Verification Trigger
                                if distance <= 25 then
                                    -- TRIP WIRE FLAGS: Shuts down the valve harvester script instantly
                                    _G.HogoThreatDetected = true
                                    _G.CleanseRoomActive = false
                                    task.wait(0.02) -- Millisecond breakdown delay to yield heartbeat welds
                ...
                task.wait(0.1) -- Rapid background threat scanning ticker frequency
            end
        end)
    else
