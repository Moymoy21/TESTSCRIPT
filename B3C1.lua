...
-- ====================================================================
-- AUTO C4 BOMB PART 2 BUTTON WRAPPER (ENHANCED & STABLE)
-- ====================================================================
createCustomButton("Section 4", "Auto C4 Bomb Part 2", "Fast tweens and plants all 6 C4 charges for Part 2", function()
    ...
    local function safeFirePromptWithLock(prompt, targetCFrame)
        ...
        -- Dito natin sinisigurong tanggihan ang line of sight check at pinapalayo ang abot
        prompt.RequiresLineOfSight = false
        prompt.MaxActivationDistance = 65 
        task.wait(0.05)
        ...
        fireproximityprompt(prompt)
        task.wait(0.7) -- Sapat na oras para mag-register ang pag-tanim
    ...
    for index, targetCFrame in ipairs(bombLocations) do
        ...
        -- Maghanap ng hanggang 3 beses sakaling maantala ang pag-load ng prompt
        for i = 1, 3 do
            targetPrompt = getClosestBombPrompt()
...
-- ====================================================================
-- INDEPENDENT AUTO SAFE ZONE TELEPORTER TOGGLE BUTTON (UPDATED)
-- ====================================================================
createToggle("Section 4", "Auto SafeZone", "Aborts farming chains and teleports character away if Hogo approaches within 25 studs", function(state)
    ...
    local labExitCFrame = CFrame.new(-3467.400, -299.855, 4349.562)

    local function notify(title, text)
        StarterGui:SetCore("SendNotification", {Title = title, Text = text, Duration = 3})
    end

    -- Helper function to safely fire prompt and bypass eyesight check
    local function safeFirePrompt(prompt)
        if not prompt then return false end
        local originalRequiresLineOfSight = prompt.RequiresLineOfSight
        local originalMaxDistance = prompt.MaxActivationDistance
    ...
    if state then
        ...
        task.spawn(function()
            while _G.AutoSafeZoneActive do
                ...
                -- ====================================================================
                -- LOCKDOWN TEXT LABEL CHECKER ("THREAT CONTAINED")
                -- ====================================================================
                local success, lockdownText = pcall(function()
                    return Workspace.Section4.Lab.CleanseRoomObjective.LOCKDOWN.LOCKTERMINAL.Screen.SurfaceGui.READY.Text
                end)
                
                if success and lockdownText == "THREAT CONTAINED" then
                    _G.HogoThreatDetected = true
                    ...
                    if rootPart then
                        ...
                        task.wait(0.01)
                        rootPart.CFrame = labExitCFrame
                        rootPart.Velocity = Vector3.new(0, 0, 0)
                        
                        notify("🚪 LAB EXIT 🚪", "Threat contained! Teleporting to Lab Exit...")
                        task.wait(1.0) -- Wait for 1 second as requested
                        
                        -- Find and fire the exit ProximityPrompt with eyesight bypassed
                        local exitPrompt = nil
                        pcall(function()
                            exitPrompt = Workspace.Section4.Lab.ExitToStairs.EXIT.ProxPart.ProximityPrompt
                        end)
                        
                        if exitPrompt then
                            safeFirePrompt(exitPrompt)
                            notify("Success", "Exit ProximityPrompt triggered successfully!")
                        else
                            notify("Error", "Exit ProximityPrompt not found!")
                        end
                ...
                -- ====================================================================
                -- 25-STUD HOGO THREAT RADAR SCANNER
                -- ====================================================================
                if rootPart then
                    for _, desc in ipairs(Workspace:GetDescendants()) do
...
-- ====================================================================
createToggle("Section 4", "Hogo ESP", "Draws visual highlights over Hogo locations across map boundaries", function(state)
